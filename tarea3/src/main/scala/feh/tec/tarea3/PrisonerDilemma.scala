...
class PrisonerDilemmaStrategicLayout extends DeterministicGameStrategicLayout{
  ...
  trait PrisonerPlayerRef extends PlayerRef{
  ...
  object A extends PrisonerPlayerRef
  object B extends PrisonerPlayerRef

  def players = Set(A, B)

  val x: Map[PlayerRef, PlayerRef#Strategy] = Map(A -> A.Betray, B -> B.Betray)
  val y: Map[PlayerRef, Utility] = Map(A -> 8, B -> 8)


  def strategicLayoutBuilder2[P1 <: PlayerRef, P2 <: PlayerRef](p1: P1, p2: P2)
  ...
  override val strategicLayout: Map[PlayersChoices, PlayersUtility] = Map(
...
//  type Ref = this.type
  ...
  def choose(choice: PrisonerDilemmaGame#Choice): Unit = ???
